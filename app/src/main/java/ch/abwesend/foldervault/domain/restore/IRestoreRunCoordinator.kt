...
/**
 * Owns the single whole-folder restore that may be in flight, so the run is not tied to the
 * lifetime of the screen that started it: a service-hosted restore keeps going while the user is
 * elsewhere, and re-entering the restore screen has to pick its progress back up.
 *
